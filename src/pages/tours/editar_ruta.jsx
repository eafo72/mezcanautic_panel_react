...
const ToursEditarRuta = () => {
  ...
  const id = localStorage.getItem("EditRutaTour");

  //getRuta
  const getRuta = async () => {
    try {
      const res = await clienteAxios.get("/admin/rutas-tour/obtenerbyruta/" + id);
      setEscala(res.data[0].escala);
      setCoordenadas(res.data[0].coordenadas);
      setTipo({
        label: res.data[0].tipo,
        value: res.data[0].tipo,
      }); 
    } catch (error) {
      console.log(error);
  ...
  const sendData = (event) => {
    ...
    if(escala == "" || escala == undefined) {
      ...
      const editRuta = async () => {
        try {
          const res = await clienteAxios({
            method: "put",
            url: "/admin/rutas-tour/set",
            data: {
              id,
              escala,
              coordenadas,
...
export default ToursEditarRuta;
